...
public class DepthCountVisitor implements Visitor<Integer> {

  @Override
  public <T extends AST> Integer visit(NodeList<T> ns) {
    int maxDepth = -1;
    for (int i = 0; i < ns.size(); i++) {
      maxDepth = Math.max(maxDepth, ns.elementAt(i).accept(this));
    }
    return maxDepth;
  }
  ...
  public Integer visit(Program n) {
    return 1 + Math.max(n.print.accept(this), n.statements.accept(this));
  }
  ...
  public Integer visit(Assign n) {
    return 1 + Math.max(n.name.accept(this), n.value.accept(this));
  }

  @Override
  public Integer visit(LessThan n) {
    return 1 + Math.max(n.e1.accept(this), n.e2.accept(this));
  }

  @Override
  public Integer visit(Conditional n) {
    return 1 + Math.max(n.e1.accept(this), Math.max(n.e2.accept(this), n.e3.accept(this)));
  }
